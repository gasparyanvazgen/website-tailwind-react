          ...
          <div className="grid gap-5 lg:grid-cols-3">
            {articleItems.map((article) => (
              <article key={article.slug} className="group rounded-2xl border border-secondary/10 bg-secondary/5 p-5 transition duration-300 hover:-translate-y-1 hover:border-secondary/20 hover:bg-secondary/10">
                ...
                <Link to={`/articles/${article.slug}`} className="group/btn mt-6 inline-flex items-center font-semibold text-accentBlueLight">
                  Read More <ArrowLongRightIcon className="ml-2 h-5 w-5 transition group-hover/btn:translate-x-1" />
                </Link>
              </article>
            ))}
          </div>
